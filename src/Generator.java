...
public class Generator {
	...
	public static void radialAuto() {
		int max = Integer.MAX_VALUE;
		// i divide by 3 to make sure to avoid integer overflow
		Point midPoint = new Point(max / 3, max / 3);
		
		Random rand = new Random();
		for (int i = 0; i < Core.genSize; i++) {
			double angle = rand.nextDouble(Math.PI * 2);
			double radius = Math.max(rand.nextInt(max/3), rand.nextInt(max/3));
			int x = (int) (Math.cos(angle) * radius + midPoint.x);
			int y = (int) (Math.sin(angle) * radius + midPoint.y);
			Core.points.add(new Point(x, y));
		}
	...
	public static void rectangularAuto(int size) {
		Random rand = new Random();
	...
	public static void rectangularAuto() {
		Random rand = new Random();
		for (int i = 0; i < Core.genSize; i++) {
			int x = rand.nextInt();
			int y = rand.nextInt();
	...
	public static void circularAuto() {
		int max = Integer.MAX_VALUE;
		// i divide by 3 to make sure to avoid integer overflow
		Point midPoint = new Point(max / 3, max / 3);
		int radius = max / 3;
